...
void Tree::createBranchesSpaceColonization(std::vector<TreeNode>& tree_nodes, TreeNode& parent, glm::mat4& model, std::vector<glm::mat4>& branchTransforms,
    float length, float radius, int depth) {
    ...
    for (size_t child_i : parent.children) {
        ...
        createBranchesSpaceColonization(tree_nodes, tree_nodes[child_i], model, branchTransforms, length, radius, depth);
    }
}

void Tree::createBranchesRootNodes(std::vector<TreeNode>& tree_nodes, glm::mat4& model, std::vector<glm::mat4>& branchTransforms,
    ...
    for (size_t i = 0; i < root_nodes; i++) {
        createBranchesSpaceColonization(tree_nodes, tree_nodes[i], model, branchTransforms, length, radius, depth);
    }
}